...
function main() {
  ...
  // ── Build daily calendar data (message count per ISO date) ──
  // hourOffset shifts each message's hour to compute which calendar day it falls on
  // in a different timezone (e.g. -8 for AEDT→MSK)
  function buildDailyCalendar(messages, hourOffset = 0) {
    const dayCounts = {}
    for (const msg of messages) {
      let d = new Date(msg.date)
      if (hourOffset !== 0 && msg.hour !== null) {
        const shifted = msg.hour + hourOffset
        if (shifted < 0) {
          d = new Date(d)
  ...
  const dailyCalendarData = {
    combined: buildDailyCalendar(allMessages),
    steven: buildDailyCalendar(stevenMsgs),
    elizabeth: buildDailyCalendar(elizabethMsgs)
  }

  // MSK timezone variant (AEDT-8)
  const dailyCalendarDataMsk = {
    combined: buildDailyCalendar(allMessages, -8),
    steven: buildDailyCalendar(stevenMsgs, -8),
    elizabeth: buildDailyCalendar(elizabethMsgs, -8)
  }
  ...
  console.log('')
  console.log('export const dailyCalendarData = ' + JSON.stringify(dailyCalendarData, null, 2))
  console.log('')
  console.log('export const dailyCalendarDataMsk = ' + JSON.stringify(dailyCalendarDataMsk, null, 2))
  console.log('')
}
